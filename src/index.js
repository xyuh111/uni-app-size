module.exports = function(schema, option) {
		...
		for (let key in style) {
			switch (key) {
				...
				case 'borderRadius':
					let value = '' + style[key];
					if (value.slice(-1) !== "%") {
						style[key] = (parseInt(style[key]) * 2).toFixed(2) + 'rpx';
					}
					break;
			}
		}
	...
	const parseDataSource = (data) => {
		...
		if (params) {
			payload =
				`${toString(payload).slice(0, -1)} ,body: ${isExpression(params) ? parseProps(params) : toString(params)}}`;
		} else {
    ...
    `;
		...
		if (data.dataHandler) {
			const {
				params,
				content
			} = parseFunction(data.dataHandler);
				...
				if (schema.methods) {
					Object.keys(schema.methods).forEach((name) => {
						const {
							params,
							content
						} = parseFunction(schema.methods[name]);
				...
				if (schema.dataSource && Array.isArray(schema.dataSource.list)) {
					...
					if (schema.dataSource.dataHandler) {
						const {
							params,
							content
						} = parseFunction(schema.dataSource.dataHandler);
				...
				if (schema.lifeCycles) {
					...
					Object.keys(schema.lifeCycles).forEach((name) => {
						const {
							params,
							content
						} = parseFunction(schema.lifeCycles[name]);
